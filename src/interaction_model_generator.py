...
from paraphraser.utterance_generator import generate
import random
from zipfile import ZipFile


class InteractionModelGenerator:
    def __init__(self, path, filename, paraphraser):
        self.path = path
    ...
    def generate(self):
        ...
        with ZipFile('download/convo.zip', 'w') as convozip:
            ...
            convozip.write('download/Covo guide.pdf')
    ...
    def generate_intents_for_post_and_put(self, schema_name, intent_name, summary):
        ...
        if len(slots) != 0:
            sample_utterances = [summary]
            for i in range(4):
                paraphrase = self.paraphraser.generate_single(summary)
                sample_utterances.append(paraphrase)
            nlp_generated = generate(summary, slots)
            sample_utterances.extend(nlp_generated)
            random.shuffle(sample_utterances)
            self.spec_intents.append({"name": intent_name, "slots": slots, "samples": sample_utterances})
    ...
    def generate_intents_for_get_and_delete(self, intent_name, slots, summary):
        ...
        if len(sl) != 0:
            ...
            for i in range(4):
                paraphrase = self.paraphraser.generate_single(summary)
                sample_utterances.append(paraphrase)
            nlp_generated = generate(summary, sl)
            sample_utterances.extend(nlp_generated)
            random.shuffle(sample_utterances)
            self.spec_intents.append({"name": intent_name, "slots": sl, "samples": sample_utterances})
